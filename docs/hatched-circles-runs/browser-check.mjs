...
try{
 ...
 await click('start');await until('document.getElementById("start").textContent === "Stop Sound"');
 const p=await evaluate('(()=>{const r=document.getElementById("grid").getBoundingClientRect(),cell=Math.min(r.width/18,r.height/7);return {x:r.x+(r.width-cell*18)/2+6.5*cell,y:r.y+(r.height-cell*7)/2+3.5*cell};})()');
 await send('Input.dispatchMouseEvent',{type:'mouseMoved',...p});await until('window.harmonyGrid.state?.filtered.length>0');await pause(150);
 check(await evaluate('getComputedStyle(document.querySelector(".legend-mark.filtered")).borderRadius === "50%"'),'Filtered legend uses a hatched circle');
 check(await evaluate('getComputedStyle(document.querySelector(".legend-mark.filtered")).borderTopWidth === "0px"'),'Filtered legend has no outline');
 await shot('filtered-circles');check(report.errors.length===0,'Shared canvas renderer has no runtime exceptions');

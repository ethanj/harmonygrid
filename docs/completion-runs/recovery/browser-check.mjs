...
try{
 ...
 await send('Page.addScriptToEvaluateOnNewDocument',{source:`const Native=window.AudioContext;window.AudioContext=class extends Native{constructor(...args){super(...args);window.testAudio=this;}};`});
 await send('Network.enable');await send('Network.setBlockedURLs',{urls:['*.sf2']});
 ...
 const begin=Date.now();await click('start');await until('document.getElementById("start").textContent==="Stop Sound"');check(Date.now()-begin<2500,'Missing palette does not block initial playing');
 check(await evaluate('window.harmonyGrid.environment.health.palette==="unavailable" && !document.getElementById("retry-palette").hidden'),'Palette failure exposes fallback and retry');
 await send('Network.setBlockedURLs',{urls:[]});await click('retry-palette');await until('window.harmonyGrid.environment.health.palette==="ready" && document.getElementById("start").textContent==="Stop Sound"');check(true,'Retry loads the palette and restarts sound');
 await evaluate('window.testAudio.suspend()');await until('document.getElementById("start").textContent==="Resume sound"');await click('start');await until('window.testAudio.state==="running"');check(true,'Suspended audio resumes through the visible control');
 const point=await evaluate('(()=>{const r=document.getElementById("grid").getBoundingClientRect();return {x:r.left+r.width/2,y:r.top+r.height/2};})()');
 await send('Input.dispatchMouseEvent',{type:'mouseMoved',...point});await until('window.harmonyGrid.state.sounding.length>0');await tap('y');await evaluate('window.dispatchEvent(new Event("blur"))');await pause(300);check(await evaluate('window.harmonyGrid.state.sounding.length>0 && window.harmonyGrid.state.controls.drone'),'Ordinary blur preserves the performance and Drone');
 await evaluate('Object.defineProperty(document,"hidden",{value:true,configurable:true});document.dispatchEvent(new Event("visibilitychange"));');await pause(1200);await evaluate('Object.defineProperty(document,"hidden",{value:false,configurable:true});document.dispatchEvent(new Event("visibilitychange"));');await pause(200);check(await evaluate('!(window.harmonyGrid.environment.health.renderOverflows??0)&&!(window.harmonyGrid.environment.health.presentationOverflows??0)'),'Synthetic visibility transitions coalesce presentation without overflow');
 await click('panic');await until('window.harmonyGrid.state.sounding.length===0');check(true,'Panic releases all notes after recovery');report.environment=await evaluate('window.harmonyGrid.environment');check(report.errors.length===0,'No browser runtime exceptions');await shot('recovered');

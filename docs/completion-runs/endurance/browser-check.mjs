...
try{
 ...
 socket.addEventListener('message',event=>{const msg=JSON.parse(event.data);if(msg.method==='Page.javascriptDialogOpening')void send('Page.handleJavaScriptDialog',{accept:true});if(msg.method==='Runtime.exceptionThrown')report.errors.push(msg.params.exceptionDetails);const p=pending.get(msg.id);if(p){pending.delete(msg.id);clearTimeout(p.timer);msg.error?p.reject(Error(JSON.stringify(msg.error))):p.resolve(msg.result);}});
 ...
 const seconds=Number(process.env.HARMONY_RUN_SECONDS??60);report.workloads=[];
 for(const scene of ['normal','expanded','retained']){
  await send('Page.navigate',{url:(process.argv[2]??'http://127.0.0.1:5174/')+'?scene='+scene});await until('!!window.harmonyGrid');await pause(250);await click('start');await until('document.getElementById("start").textContent==="Stop Sound"');
  await evaluate(`(()=>{const e=document.getElementById('sound');e.value='piano';e.dispatchEvent(new Event('change'));})()`);
  if(scene==='retained'){await tap('f');await tap('t');await evaluate(`document.querySelector('[data-control="repeat"]').click()`);}
  const g=await evaluate(`(()=>{const r=document.getElementById('grid').getBoundingClientRect(),cols=${scene==='normal'?16:scene==='expanded'?24:18},rows=${scene==='normal'?12:scene==='expanded'?16:7},cell=Math.min(r.width/cols,r.height/rows);return {left:r.x+(r.width-cols*cell)/2,top:r.y+(r.height-rows*cell)/2,cell,cols,rows};})()`);
  const before=await evaluate('window.harmonyGrid.environment.audio.currentTime'),begin=Date.now();let moves=0;
  while(Date.now()-begin<seconds*1000){
   await send('Input.dispatchMouseEvent',{type:'mouseMoved',x:g.left+(.5+moves%g.cols)*g.cell,y:g.top+(.5+(moves*7)%g.rows)*g.cell});moves++;await pause(10);
   if(moves%500===0)console.log(scene+' '+Math.round((Date.now()-begin)/1000)+'s');
  }
  const raw=await exportTrace(),data=JSON.parse(raw),env=data.environment;
  check(!env.audio.fault&&env.audio.currentTime-before>seconds*.9,scene+': audio clock continues throughout sustained dragging');
  check(env.health.queueOverflows===0&&env.health.nonFiniteSamples===0&&!(env.health.presentationOverflows??0)&&!(env.health.renderOverflows??0),scene+': finite audio and bounded queues');
  check(data.events.filter(e=>e.kind==='commit').length>10&&data.events.filter(e=>e.kind==='draw').length>10,scene+': audio and visualization still advance at the end');
  const draws=data.events.filter(e=>e.kind==='draw').map(e=>e.duration).sort((a,b)=>a-b);
  report.workloads.push({scene,wallSeconds:(Date.now()-begin)/1000,moves,retainedEvents:data.events.length,olderEventsReplaced:data.dropped,drawSamples:draws.length,p50:draws[Math.floor(draws.length*.5)],p95:draws[Math.floor(draws.length*.95)],p99:draws[Math.floor(draws.length*.99)],maximum:draws.at(-1),environment:env});
  await writeFile(join(root,scene+'-trace.json'),raw);await shot(scene);await click('panic');await until('window.harmonyGrid.state.sounding.length===0');check(true,scene+': deliberate panic releases every logical note');
 }

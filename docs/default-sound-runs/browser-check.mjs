...
try{
 ...
 await until('window.harmonyGrid.environment.audio && !document.getElementById("start").disabled');
 if(process.argv.includes('--autoplay')){
  await until('window.harmonyGrid.environment.audio.state === "running"');
  check(true,'Sound starts without a gesture when autoplay is allowed');
 }else{
  ...
  await send('Input.dispatchMouseEvent',{type:'mousePressed',...p,button:'left',buttons:1,clickCount:1});
  await send('Input.dispatchMouseEvent',{type:'mouseReleased',...p,button:'left',buttons:0,clickCount:1});
  await until('document.getElementById("start").textContent === "Stop Sound"');
  await until('window.harmonyGrid.state?.sounding.length > 0');
  check(true,'First grid gesture unlocks sound and plays notes');
 }
 await click('start');await until('window.harmonyGrid.environment.audio === null');
 await click('grid');await pause(300);
 check(await evaluate('window.harmonyGrid.environment.audio === null'),'Explicit Stop stays off after a grid gesture');

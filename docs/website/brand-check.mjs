...
const chrome=spawn('/Applications/Google Chrome.app/Contents/MacOS/Google Chrome',['--headless',...(process.argv.includes('--autoplay')?['--autoplay-policy=no-user-gesture-required']:[]),'--no-first-run','--no-default-browser-check','--disable-extensions','--disable-background-networking','--mute-audio','--remote-debugging-port=0',`--user-data-dir=${profile}`,'about:blank'],{stdio:['ignore','ignore','pipe']});
...
try{
 ...
 await until('document.querySelector(".brand img")?.complete');
 check(await evaluate('document.querySelector(".brand img").naturalWidth > 0'),'Original icon loads on play page');
 check(await evaluate('new URL(document.querySelector(".brand").href).pathname === "/"'),'Play title links to homepage');
 await send('Page.captureScreenshot',{format:'png'}).then(r=>writeFile(join(root,'brand.png'),Buffer.from(r.data,'base64')));
 await evaluate('document.querySelector(".brand").click()');await until('!!document.querySelector("#hero-grid")');
 check(true,'Play branding navigates home');
 await evaluate(`document.querySelector('a[href="about.html"]').click()`);
 await until('!!document.querySelector("a[download]")');
 check(await evaluate('document.querySelector("a[download]").getAttribute("href") === "harmonygrid.pdf"'),'About provides original PDF download');
 check(report.errors.length===0,'No runtime exceptions');

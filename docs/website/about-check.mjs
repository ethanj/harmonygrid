...
try{
 ...
 await send('Page.navigate',{url:process.argv[2]??'http://127.0.0.1:5173/docs/website/'});await until('document.querySelectorAll(".history-hero img").length === 2');
 ...
 check(await evaluate('document.documentElement.scrollWidth <= innerWidth'),'Desktop fits viewport');
 await shot('about-desktop');
 await send('Emulation.setDeviceMetricsOverride',{width:390,height:844,deviceScaleFactor:1,mobile:false});
 check(await evaluate('document.documentElement.scrollWidth <= innerWidth'),'Mobile fits viewport');await shot('about-mobile');
 check(report.errors.length===0,'Landing page has no runtime exceptions');

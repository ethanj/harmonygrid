...
try{
 ...
 await send('Page.navigate',{url:process.argv[2]??'http://127.0.0.1:5173/docs/website/'});await until('document.querySelectorAll("#notes > g").length === 30');
 ...
 check(await evaluate('document.documentElement.scrollWidth <= innerWidth'),'Mobile fits viewport');await shot('mobile');

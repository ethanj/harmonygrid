...
try{
 ...
 await click('start');await until('document.getElementById("start").textContent === "Stop Sound" && !document.getElementById("start").disabled');
 check(true,'Running sound exposes an enabled Stop Sound button');
 const p=await evaluate('(()=>{const r=document.getElementById("grid").getBoundingClientRect();return {x:r.x+r.width/2,y:r.y+r.height/2};})()');
 await send('Input.dispatchMouseEvent',{type:'mouseMoved',...p});await until('window.harmonyGrid.state?.sounding.length > 0');await tap('y');await until('window.harmonyGrid.state?.controls.drone');
 const settings=await evaluate('JSON.stringify(window.harmonyGrid.state.settings)');
 await click('start');await until('document.getElementById("start").textContent === "Start sound" && !window.harmonyGrid.environment.audio');
 await until('window.harmonyGrid.state?.sounding.length===0 && !window.harmonyGrid.state?.controls.drone');check(true,'Stop closes audio and clears retained notes');
 await send('Input.dispatchMouseEvent',{type:'mouseMoved',x:p.x+20,y:p.y});await pause(300);
 check(await evaluate('!window.harmonyGrid.environment.audio && window.harmonyGrid.state?.sounding.length===0'),'Dragging while stopped does not restart audio');
 await shot('stopped');await click('start');await until('document.getElementById("start").textContent === "Stop Sound"');
 check(await evaluate('JSON.stringify(window.harmonyGrid.state.settings)')===settings,'Restart preserves instrument settings');
 await send('Input.dispatchMouseEvent',{type:'mouseMoved',...p});await until('window.harmonyGrid.state?.sounding.length>0');await pause(600);
 check(await evaluate('window.harmonyGrid.environment.health.peak>0 && !window.harmonyGrid.environment.audio.fault'),'Restart plays again');await shot('playing');
 await click('start');await until('!window.harmonyGrid.environment.audio');check(report.errors.length===0,'No browser runtime exceptions');

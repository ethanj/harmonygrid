...
try{
 ...
 for(const [width,height] of [[1280,1100],[1600,1200]]){
 await send('Emulation.setDeviceMetricsOverride',{width,height,deviceScaleFactor:1,mobile:false});
 await send('Page.navigate',{url:new URL('index.html',import.meta.url).href});await pause(150);
 await writeFile(join(root,'controls-'+width+'.png'),Buffer.from((await send('Page.captureScreenshot',{format:'png',captureBeyondViewport:false})).data,'base64'));
 }

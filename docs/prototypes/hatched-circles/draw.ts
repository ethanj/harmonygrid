import {pitchClass,gridPitch} from '../../../src/performance/harmony';
import type {Snapshot} from '../../../src/performance/model';
import {pianoKeys,type GridGeometry} from '../../../src/render/geometry';
...
function hatch(c:Context,cx:number,cy:number,r:number){
  ...
  c.save();c.beginPath();c.arc(cx,cy,r,0,Math.PI*2);c.clip();c.strokeStyle='#b78b53';c.lineWidth=.8;
...
export function drawGrid(c:Context,g:GridGeometry,s:Snapshot|null):void {
  ...
  const sound=new Set(s?.sounding.map(n=>n.pitch)),played=new Set(s?.capture?.selected??s?.played),filtered=new Set(s?.capture?[]:s?.filtered),raw=new Set(s?.midi),pcs=new Set(s?.midi.map(pitchClass));
  ...
  for(let row=0;row<g.rows;row++)for(let column=0;column<g.columns;column++){
    ...
    if(played.has(pitch)&&!filter){
      c.strokeStyle='#82b090';c.lineWidth=1.4;c.strokeRect(x+1,y+1,cell-2,cell-2);
    }
    if(sc&&!unavailable||sounding)circle(c,x+cell/2,y+cell/2,cell*.35,sounding?'#a0dfc8':null,sounding?'#a0dfc8':isRoot?'#a0c6a4':'#45684d');
    if(isRoot&&!unavailable)circle(c,x+cell/2,y+cell/2,cell*.40,null,'#9ac2a0');
    if(filter){hatch(c,x+cell/2,y+cell/2,cell*.35);circle(c,x+cell/2,y+cell/2,cell*.35,null,'#b68d5a');}
    if(raw.has(pitch)){
    ...
    c.fillStyle=sounding?'#123626':unavailable?'#58695e':filter?'#efc693':isRoot?'#c4dfbf':sc?'#a6bba5':'#7f947f';
...
export function drawPiano(c:Context,width:number,height:number,s:Snapshot|null,low=48,high=84,maximum=127):void {
  ...
  const sound=new Set(s?.sounding.map(n=>n.pitch)),filtered=new Set(s?.capture?[]:s?.filtered),raw=new Set(s?.midi);
  const root=s?.capture?(s.capture.reference===null?-1:pitchClass(s.capture.reference)):s?.settings.root??0,mode=s?.capture?Array.from({length:12},(_,i)=>i):s?.settings.mode??[0,2,4,5,7,9,11];
  for(const k of pianoKeys(width,height,low,high)){
    const sounding=sound.has(k.pitch),filter=filtered.has(k.pitch),sc=mode.includes(pitchClass(k.pitch-root));
    c.fillStyle=sounding?'#a0dfc8':k.pitch>maximum?(k.black?'#303b32':'#3a463c'):k.black?'#18221b':'#c3ccbe';c.fillRect(k.x+.7,0,k.width-1.4,k.height);c.strokeStyle='#526650';c.lineWidth=1;c.strokeRect(k.x+.7,0,k.width-1.4,k.height);
    if(filter){const radius=Math.min(k.width*.36,k.height*.24),cx=k.x+k.width/2,cy=k.height*.42;hatch(c,cx,cy,radius);circle(c,cx,cy,radius,null,'#b68d5a');}
    ...
    if(sc&&k.pitch<=maximum)circle(c,k.x+k.width/2,k.height-22,3.8,pitchClass(k.pitch)===root?'#365541':null,sounding||!k.black?'#486b51':'#9cae97');
    if(pitchClass(k.pitch)===0||sounding||filter){c.fillStyle=sounding||!k.black?'#294332':'#cfbc93';c.font='10px -apple-system, sans-serif';c.textAlign='center';c.textBaseline='alphabetic';c.fillText(noteName(k.pitch),k.x+k.width/2,k.height-7);}
  }
}

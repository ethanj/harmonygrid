...
try{
 ...
 for(const state of ['chord','mode','invalid']){
  await send('Emulation.setDeviceMetricsOverride',{width:1280,height:800,deviceScaleFactor:1,mobile:false});
  await send('Page.navigate',{url:new URL('index.html?scene=4'+(state==='mode'?'&target=mode':'')+(state==='invalid'?'&invalid=1':''),import.meta.url).href});await pause(300);
  check(await evaluate('document.querySelector(".detail").getBoundingClientRect().bottom <= innerHeight'),state+' fits viewport');

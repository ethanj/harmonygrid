...
try{
 ...
 for(const [width,height] of [[1280,800],[1600,1000]])for(const scene of ['default','fifths','octaves','ceiling','register','invalid']){
  await send('Emulation.setDeviceMetricsOverride',{width,height,deviceScaleFactor:1,mobile:false});
  await send('Page.navigate',{url:new URL('index.html?scene='+scene,import.meta.url).href});await pause(200);
  check(await evaluate('document.getElementById("apply").getBoundingClientRect().bottom <= innerHeight'),scene+' actions fit '+width);
  check(await evaluate('document.documentElement.scrollWidth <= innerWidth'),scene+' no horizontal overflow '+width);
  await writeFile(join(root,scene+'-'+width+'.png'),Buffer.from((await send('Page.captureScreenshot',{format:'png',captureBeyondViewport:false})).data,'base64'));
 }
 await send('Page.navigate',{url:new URL('index.html',import.meta.url).href});await pause(200);
 await evaluate('document.querySelector(`[data-preset="5,7"]`).click()');
 check(await evaluate('document.getElementById("layout-name").textContent === "5 × 7" && document.getElementById("applied").textContent.startsWith("4 × 3")'),'Draft preview leaves applied layout unchanged');
 await click('cancel');check(await evaluate('document.getElementById("horizontal").value === "4"'),'Cancel restores values');
 await evaluate('document.querySelector(`[data-preset="1,12"]`).click()');await click('apply');
 check(await evaluate('document.getElementById("applied").textContent.startsWith("1 × 12") && document.getElementById("apply").disabled'),'Apply updates preview baseline');
 await evaluate('document.getElementById("horizontal").value="13";document.getElementById("horizontal").dispatchEvent(new Event("input"))');
 check(await evaluate('document.getElementById("apply").disabled && document.getElementById("error").textContent.includes("1 to 12")'),'Invalid interval blocks Apply');
 await click('cancel');
 await evaluate('document.getElementById("maximum").value="84";document.getElementById("maximum").dispatchEvent(new Event("input"))');
 check(await evaluate('document.querySelectorAll(`[data-unavailable="true"]`).length > 0'),'Pitch ceiling dims unavailable cells');
